    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-[#161616] border border-[#262626] rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-[#262626] flex items-center justify-between">
          <div>
            <h3 className="font-extrabold text-base text-white">Tambah Update Live Timeline</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="text-[11px] font-semibold text-zinc-400 block mb-2">Pilih Quick Milestone:</label>
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => applyPreset('PROGRESS', '🚀 Running update: Trade running +50 point.', 50)}
                className="text-left p-2.5 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-[#F5B942]/50 hover:bg-zinc-800 text-xs font-semibold text-zinc-200 transition-all cursor-pointer"
              >
                🚀 +50 Point Milestone
              </button>
              ...
              <button
                ...
                className="text-left p-2.5 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-blue-500/50 hover:bg-zinc-800 text-xs font-semibold text-blue-300 transition-all cursor-pointer"
              >
              ...
              <button
                ...
                className="text-left p-2.5 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-emerald-500/50 hover:bg-zinc-800 text-xs font-semibold text-emerald-300 transition-all cursor-pointer"
              >
                💰 Take Partial Profit
              </button>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2.5">
            <div>
              <label className="text-[11px] font-semibold text-zinc-400 block mb-1">Harga Saat Ini</label>
              <input
                type="number"
                step="any"
                value={currentPrice}
                onChange={(e) => setCurrentPrice(e.target.value)}
                className="w-full bg-[#1C1C1C] border border-[#262626] focus:border-[#F5B942] rounded-xl px-3 py-2 text-xs font-mono font-bold text-white focus:outline-none"
              />
            </div>

            <div>
              <label className="text-[11px] font-semibold text-zinc-400 block mb-1">Floating Points</label>
              <input
                type="number"
                step="any"
                value={floatingPoints}
                onChange={(e) => setFloatingPoints(e.target.value)}
                className="w-full bg-[#1C1C1C] border border-[#262626] focus:border-[#F5B942] rounded-xl px-3 py-2 text-xs font-mono font-bold text-emerald-400 focus:outline-none"
              />
            </div>

            <div>
              <label className="text-[11px] font-semibold text-zinc-400 block mb-1">Floating ($)</label>
              <input
                type="number"
                step="any"
                value={floatingMoney}
          ...
          <div className="pt-3 border-t border-[#262626] flex items-center justify-end space-x-3">
            ...
            <button
              type="submit"
              className="flex items-center space-x-2 px-5 py-2.5 rounded-xl text-xs sm:text-sm font-bold bg-[#F5B942] hover:bg-[#efa823] text-black active:scale-95 transition-all cursor-pointer"
            >
              <Send className="w-3.5 h-3.5" />
              <span>Kirim Update ke Feed</span>
            </button>
          </div>
